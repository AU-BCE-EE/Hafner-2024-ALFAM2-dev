...
dd <- dat[sid %in% c(1, 2, 3, 7, 10, 11) & (app.mthd != 'cs' | pars != 'ps01') & pars != 'ALFAM', ]
t0 <- copy(dd)
t0[, `:=` (ct = 0, er = 0)]
...
ggsave2x('../plots/sens_curves', height = 8, width = 6.6)

dd <- dat[sid == 1 & (app.mthd != 'cs' | pars != 'ps01') & pars != 'ALFAM', ]
t0 <- copy(dd)
t0[, `:=` (ct = 0, er = 0)]
t0 <- t0[!duplicated(t0[, .(sid, app.mthd, pars, ct, er)]), ]
dd <- rbind(t0, dd)
...
ggplot(dd, aes(ct, er, colour = pars.nm)) +
  ...
  theme_bw() +
  theme(legend.position = 'top') +
  facet_grid(. ~ app.mthd.nm) +
  labs(x = 'Time since application (h)', y = 'Cum. emission (frac. TAN)', colour = 'Parameter set')
ggsave2x('../plots/sens_curves_pres', height = 3, width = 9)
