...
# Get quantiles
qdat <- pdat[!is.na(dedx) & parset != '3', .(n = length(dedx), dedx10 = quantile(dedx, 0.10), dedx90 = quantile(dedx, 0.90), dedxmd = median(dedx)), 
...
# Get quantiles
qamdat <- pamdat[!is.na(rr) & parset != '3', .(n = length(rr), rr10 = quantile(rr, 0.10), rr90 = quantile(rr, 0.90), rrmd = median(rr)), 
             by = .(ct, man.dm, wind.2m, app.rate.ni, man.ph, tan.app, man.source, air.temp, app.mthd, id, set, xval, id.pred)]
