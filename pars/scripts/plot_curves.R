...
pdf('../plots-curves/emis_curves.pdf', height = 11, width = 8.5)
  ...
  for (i in unique(idat1[, pmid])) {
    ...
    lines(er.pred ~ cta, data = dd, type = 'l', col = 'red') 
  }
dev.off()

pdf('../plots-curves/flux_curves.pdf', height = 11, width = 8.5)
  par(mfrow = c(4, 3))
  for (i in unique(idat1[, pmid])) {
    dd <- dpreds[pmid == i & pars == 'ps3', ]  
    dd <- rbind(dd, data.table(cta = 0, er = 0, er.pred = 0), fill = TRUE)
    dd <- dd[order(cta), ]
    plot(j ~ cta, data = dd, type = 's', col = 'blue', 
         main = paste('pmid', i, 'inst', dd[2, inst], dd[2, app.mthd.nm]), 
         xlab = 'Time after application (h)', ylab = 'Interval flux (kg/ha-h)',
         ylim = c(0, max(na.omit(c(dd[, j], dd[, j.pred])))))
    points(j ~ cta, data = dd, col = 'blue') 
    lines(j.pred ~ cta, data = dd, type = 's', col = 'red') 
  }
dev.off()
