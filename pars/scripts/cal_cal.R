...
pars.cal <- pars.cal[grepl('^int', names(pars.cal))]
pars.cal['int.r5'] <- -2.0
...
ps <- 'c1'
print(ps)
pbsth <- mods$'bsth0'$coef
names(pbsth) <- gsub('^int', 'app.mthd.bsth', names(pbsth))
pbc <- mods$'bc0'$coef
names(pbc) <- gsub('^int', 'app.mthd.bc', names(pbc))
pos <- mods$'os0'$coef
names(pos) <- gsub('^int', 'app.mthd.os', names(pos))
pcs <- mods$'cs0'$coef
names(pcs) <- gsub('^int', 'app.mthd.cs', names(pcs))
pars.cal <- c(pbsth, pbc, pos, pcs)
