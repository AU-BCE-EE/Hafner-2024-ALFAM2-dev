...
dd <- subset(parsl, pars %in% c('ps1', 'ps2', 'p1', 'w2', 'w3', 'w4'))
dd[, pars.nm := factor(pars, levels = c('ps1', 'ps2', 'p1', 'w2', 'w3', 'w4'), labels = c(1:3, 'w2', 'w3', 'w4'))]
dd3 <- subset(dd, pars == 'p1')
ggplot(dd, aes(reorder(parameter, ppar), value, colour = pars.nm, group = pars)) +
  geom_hline(yintercept = 0, lty = 2) +
  geom_point(size = 2, shape = 1) +
  geom_point(data = dd3, size = 2) +
  geom_line() +
  facet_wrap(~ ppar, scale = 'free') +
  labs(x = 'Secondary parameter', y = 'Value', colour = 'Parameter\nset') +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
ggsave2x('../plots-pars/pars_wt', height = 6, width = 6)

dd <- subset(parsl, pars %in% c('ps1', 'ps2', 'p1', 'nullA', 'nullB', 'nullC'))
dd[, pars.nm := factor(pars, levels = c('ps1', 'ps2', 'p1', 'nullA', 'nullB', 'nullC'), labels = c(1:3, 'Null A', 'Null B', 'Null C'))]
